...
void Latency::sendMessage(int clientID, std::string message, PacketType type) {
    ...
    if (gameObjectsSet && (buffsize > ((double) MAX_ALLOWED_DELAY / averageClientLatency))) {
...
std::string Latency::addTimestamp(std::string message) {
    ...
    if (!parsedSuccess)
        printf("WARNING: %d- Unsuccessful parse when adding time stamp.\n", ID);
    ...
    // correct for difference in client clock
    //     millis -= offset;
    //    cout << offset << endl;
    ostringstream stamp;
    stamp << millis;
...
void Latency::sendBufferMessage() {
    ...
    //
    //    }
    //    if (curPlayer->getAssignedClientID() == 0)
    //       r += 50;
...
void Latency::handleIncomingMessage(int clientID, std::string message) {
    ...
    clientLatency += currMillis - (timeStamp - offset);
    // if (clientID == 0)
    //         clientLatency += (currMillis - (timeStamp + 0));
    //     else
...
//void Latency::setClientLatency(int clientID) {
//    if ()
//}

long long Latency::getClockOffset() {
    return offset;
}
